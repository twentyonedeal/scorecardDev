...
<style>
	...
	:root {
  		color-scheme: light; /* Forces the browser to treat the page as 'light' mode */
		}
	/* Add this to your styles.css */
	/* This is what makes the background purple */
	
	
	header {
    background-color: #4B0082;
    /* This adds padding ONLY at the top, specifically 
       matching the height of the iPhone status bar */
    padding-top: env(safe-area-inset-top); 
}
	
	body {
		/* Pushes all content down so it starts below the status bar */
		padding-top: env(safe-area-inset-top);
		}

	/* Create a top bar that covers the status area */
	body::before {
    	content: "";
    	position: fixed;
    	top: 0;
    	left: 0;
    	width: 100%;
    	/* The magic variable that matches the height of the notch/clock */
    	height: env(safe-area-inset-top); 
    	background-color: #4B0082; /* Your deep purple */
    	z-index: 9999; /* Keeps it above everything else */
		pointer-events: none; /* So it doesn't block clicks/taps */
		}

	/* Push your actual content down so it doesn't hide behind the purple shelf */
	header, .container {
		padding-top: env(safe-area-inset-top);
		}
    ...
    * { box-sizing: border-box; }
    ...
    .calc-btn { padding: 15px 0; background: #612dd8; color: white; border-radius: 8px; border: none; font-weight: bold; font-size: 1.1rem; }
    .calc-btn.op { background: #d8612d; }
	.calc-btn.back { background: #ee0b2f; }
	.calc-btn.clear { background: #f13c59; }
    ...
    .controls2 { display: flex; justify-content: space-between; align-items: center; margin: 0px 0 10px 0; padding-bottom: 5px; font-weight: 800; text-transform: uppercase; font-size: 0.65rem; letter-spacing: 1px; color: #95a5a6; }
	...
	/* The actual movement: left to right quickly */
	@keyframes shake {
	...
	.shake-error {
    	...
    	border: 2px solid #e74c3c !important; /* Optional: flash the border red too */
		}
		
	/* Universal Drag/Selection Fixes */
    * { -webkit-touch-callout: none; -webkit-user-select: none; user-select: none; }
    input, select, .editable-name { -webkit-user-select: text; user-select: text; pointer-events: auto !important; }
		
	.drag-handle { width: 40px; text-align: center; color: #95a5a6; font-size: 20px; cursor: grab; touch-action: none; }
    .sortable-ghost { opacity: 0.5; background: #3498db !important; }
        
	/* Prevents pull-to-refresh */
	html, body {
		/* Disables pull-to-refresh and the "rubber-band" bounce */
...
<body>
	...
	<div class="notch_container">
		<h1 style="text-align:center; font-size: 1.5rem; margin:3rem 0 10px 0;"></h1>
    </div>
	
	<div class="container">
		<h2 style="text-align:center; font-size: 1.5rem; margin:0 0 5px 0;">Jay's Ultimate Scorecard</h2>
    <div style="text-align:center;  font-size: .75rem; margin:0 0 5px 0;">V250</div>
    ...
    <div style="display:grid; grid-template-columns: 1fr 1fr 44px 44px; gap:8px; margin-bottom:15px;">
        <button onclick="openSaveModal()" style="background:#3498db; color:white; border:none; border-radius:8px; padding:12px;">💾 Save</button>
        <button onclick="openLibrary()" style="background:#9b59b6; color:white; border:none; border-radius:8px; padding:12px;">📚 Lib</button>
        <button onclick="toggleManualMode()" id="seatingBtn" style="background:#34495e; color:white; border:none; border-radius:8px; padding:12px;">🪑</button>
        <button onclick="toggleDarkMode()" style="background:#34495e; color:white; border:none; border-radius:8px; padding:12px;">🌙</button>
    </div>
    ...
    <div class="section-title">
		<span id="leaderboard-title" data-label="🏆 Leader Board"></span>
		<div class="controls">
			<label>
				<input type="checkbox" style="cursor: pointer;" id="toggleNumbers" onchange="updateVisibility()"> 
				Last Round Scores
			</label>
		</div>
	</div>
    <div class="section-title2">
	 	<span id="leaderboard-title2" ></span>
		<div class="controls2">
			<div id="seating_container" style="display:none;">
			<label>
				<input type="checkbox" style="cursor: pointer;" id="toggleSeating" onchange="updateVisibility2()"> 
				Seating Locked
			</label>
			</div>
		</div>
    ...
    <div id="batchAction" style="display:none; margin-top:15px;">
        <button onclick="submitBatchRound()" style="width:100%; background:#27ae60; padding:15px; font-size:1.1rem; color:white; border:none; border-radius:10px; font-weight:bold;">➕ Submit Scores</button>
    </div>
    <div class="section-title">
...
<div id="calcModal" class="modal">
    <div class="modal-content">
        ...
        <div class="calc-grid">
            ...
            <button class="calc-btn" onclick="calcPush('0')">0</button>
            <button class="calc-btn clear" onclick="calcClearAll()">C</button>
            <button class="calc-btn back" onclick="calcClear()">⌫</button>
            <button class="calc-btn op" style="background:#27ae60; color:white;" onclick="calcSolve()">=</button>
...
<div id="error-modal" class="modal"><div class="modal-content"
style="max-width:380px; border-top: 5px solid #e74c3c; display: flex; flex-direction:column; gap:8px; margin-top:15px;">
        <h3 style="color: #e74c3c;">⚠️ Invalid input detected!</h3>
        <p>Scores must be whole numbers (positive or negative) without decimals or special symbols. Please fix these entries:
		
		</p>
		<button onclick="closeModal('error-modal')" style="width:100%; background:#3498db;  padding:15px .2rem; font-size:1rem; color:black; border:none; border-radius:10px;">Fix it</button>

    </div>
</div>

<!-- The Error Modal Backdrop -->
<div id="errorModal" style="display: none; position: fixed; z-index: 9999; left: 0; top: 0; width: 100%; height: 100%; backgroundColor: rgba(0,0,0,0.5); justify-content: center; align-items: center;">
    <!-- Modal Content Box -->
    <div style="background-color: white; padding: 20px; border-radius: 8px; max-width: 90%; width: 400px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        <h3 style="color: #dc3545; margin-top: 0;">⚠️ Input Error</h3>
        <p style="margin-bottom: 15px; color: #333;">Scores must be positive or negative whole numbers without decimals.</p>
        
        <!-- Error List Target -->
        <ul id="modalErrorList" style="color: #666; padding-left: 20px; line-height: 1.5; margin-bottom: 20px;"></ul>
        
        <!-- Close Button -->
        <button type="button" onclick="closeErrorModal()" style="width: 100%; padding: 10px; background-color: #dc3545; color: white; border: none; border-radius: 4px; font-weight: bold; cursor: pointer;">Fix Entries</button>
    </div>
	</div>
...
<h3>Save Game</h3>
<input type="text" id="saveNameIn" placeholder="NewGame" onkeydown="if(event.key==='Enter') doSave(false)">
...
<h3>Edit Score</h3>
 <input type="text" pattern="-[0-9]*|[0-9]*" inputmode="numeric" id="editIn" style="width:100%;
...
<script>
	...
	const statusEl = document.getElementById('syncStatus');
        statusEl.innerText = "✅ Cache OK";
                
    const lb = document.getElementById('leaderboard-title');
		lb.innerText = lb.dataset.label; // Sets the text to "🏆 Leader Board"
	let isManualMode = false, draggingRow = null, renamingPlayerId = null;
    ...
    function updateStatus() {
        ...
        if (navigator.onLine) { tag.innerText = ""; tag.className = "online"; }
        else { tag.innerText = ""; tag.className = "offline"; }
    }
    ...
    const colors = ['#42f5dd', '#e74c3c', '#2ecc71', '#f1c40f', '#9b59b6', '#e67e22', '#f0f70f', '#D20BE3', '#42c8f5'];
	let availableColors = [];

	function getRandomColor() {
		// 1. If the "deck" is empty, refill and shuffle it
		if (availableColors.length === 0) {
			availableColors = [...colors].sort(() => Math.random() - 0.5);
			}

		// 2. Remove and return the last color from the shuffled deck
  ...
  //  }
    ...
    function calcClear() { let d=document.getElementById('calcDisplay'); d.innerText = d.innerText.slice(0,-1) || "0"; }
    function calcClearAll() { document.getElementById('calcDisplay').innerText = '0'; }

	function calcSolve() {
    const display = document.getElementById('calcDisplay');
    const modalContent = document.querySelector('#calcModal .modal-content');
    try {
        // 1. Try to calculate the result
        // We replace '×' with '*' for math, then evaluate
        let result = eval(display.innerText.replace(/×/g, '*'));

        // 2. Check if the result is actually a valid number
        if (result === undefined || isNaN(result)) {
            throw new Error("Invalid");
        }

        // 3. Success! Update the display
        display.innerText = result;

    } catch (e) {
        // 4. Instead of writing "Error" in the box, show the warning modal
		// 1. Add the shake class
        modalContent.classList.add('shake-error');

		// 3. Remove the class after 300ms so it can shake again next time
        setTimeout(() => {
    ...
    try {
        ...
        // 3. Success! Update the display
        {display.innerText = result;
		let r = document.getElementById('calcDisplay').innerText;
		document.getElementById(activeCalcId).value = r;
	...
	//function calcSolve() { try { document.getElementById('calcDisplay').innerText = eval(document.getElementById('calcDisplay').innerText.replace(/×/g, '*')); } catch { document.getElementById('calcDisplay').innerText = "Error"; } }
	...
	function submitBatchRound() {
		...
		// First Pass: Validate ALL inputs to collect errors
		players.forEach(p => {
        ...
        // Skip completely empty fields safely
        if (rawValue === "") return;

        // Strict Check: Optional minus sign followed strictly by digits
        const isValidWholeNumber = /^-?[0-9]+$/.test(rawValue);

        if (!isValidWholeNumber) {
            const displayName = p.name ? p.name : `Player ${p.id}`;
            errors.push(`${displayName}: "${rawValue}"`);
			}
		});

		// If errors exist, open the custom modal and HALT execution
		if (errors.length > 0) {
			showErrorModal(errors);
			return; // Stops the function from saving or clearing anything
			}

		// Second Pass: Calculate and process if everything is valid
		players.forEach(p => {
        const el = document.getElementById(`b-${p.id}`);
        if (!el) return;
        ...
        p.score += v;
        nr.scores[p.id] = v;
        el.value = ""; // Clear input safely
        added = true;
    });

    // Save the round if data was modified
    if (added) {
        rounds.push(nr);
	...
	function showErrorModal(errorArray) {
		const modal = document.getElementById('errorModal');
		const list = document.getElementById('modalErrorList');
    
		// Clear old errors and inject new ones safely as text fragments
		list.innerHTML = "";
		...
		// Display the modal using Flexbox layout to center it
    ...
    //function saveEdit() {
	//	const el = document.getElementById(`editIn`).value;
    //    const rawValue = el.value.trim();
...
function saveEdit() {
    const inputEl = document.getElementById('editIn');
    if (!inputEl) return;

    const rawValue = inputEl.value.trim();

    // Strict Check: Optional minus sign followed strictly by digits (must not be empty or just "-")
    const isValidWholeNumber = /^-?[0-9]+$/.test(rawValue);

    if (!isValidWholeNumber) {
        // Find player name to make the error message clear
        const p = players.find(x => x.id === editState.pi);
        const displayName = p && p.name ? p.name : "Player Value";
        
        // Open your error modal and halt the save process
        showErrorModal([`${displayName}: "${rawValue}"`]);
        return; 
    }

    // If it passes validation, safely parse the number
    const nv = parseInt(rawValue, 10);
    const p = players.find(x => x.id === editState.pi);
    
    if (p) {
        // Recalculate player's total score based on the edit adjustment
	...
	function toggleManualMode() {
		// Define the new text or HTML content
		const isChecked2 = document.getElementById('toggleSeating').checked;
		const leaderboard = document.getElementById("leaderboard-title");
		const showlocked = document.getElementById('seating_container');

		//document.getElementById("leaderboard-title").innerText = newTitle;

		// Check if the current data-label is the default value
		if (leaderboard.dataset.label === "🏆 Leader Board") {
			// If it is, change it to something else
			leaderboard.dataset.label = "🪑Table Seating";
			leaderboard.innerText = "🪑 Table Seating";
			showlocked.style.display = 'inline-block';

			}
		else {
			// If it's NOT the default, toggle it back
			leaderboard.dataset.label = "🏆 Leader Board";
			leaderboard.innerText = "🏆 Leader Board";
			showlocked.style.display = 'none';
			}
			
//"🥇 Top Players";

	//	const newTitle = "🥇 Top Players";
		// Update the content of the span	
	//	document.getElementById("leaderboard-title").innerHTML = newTitle;

        isManualMode = !isManualMode;
        document.getElementById('seatingBtn').style.background = isManualMode ? "#3498db" : "#34495e";
        render();
    }
 function initDragEvents(row) {
		console.log("initDragEvent_SeatingModeOn:",SeatingModeOn);
        if(SeatingModeOn) {

		const start = (e) => {
            if (!isManualMode || e.target.closest('.editable-name') || e.target.closest('input') || e.target.closest('button')) return;
            draggingRow = row;
            row.classList.add('sortable-ghost');
        };

        const move = (e) => {
            if (!isManualMode || !draggingRow) return;
            ...
            if (targetRow && targetRow !== draggingRow && targetRow.parentNode === draggingRow.parentNode) {
                ...
                targetRow.parentNode.insertBefore(draggingRow, next ? targetRow.nextSibling : targetRow);
            }
        };

        const end = () => {
            if (!draggingRow) return;
            draggingRow.classList.remove('sortable-ghost');
            Array.from(document.querySelectorAll('#leaderboard tr')).forEach((r, i) => {
                const p = players.find(x => x.id == r.getAttribute('data-id'));
                if(p) p.position = i;
            });
            draggingRow = null;
            sync();
    ...
    function render() {
		...
		console.log("isCheckedinrender: ",isChecked);
		const listEl = document.getElementById('leaderboard');
        const rankedRef = [...players].sort((a,b) => lowScoreWins ? a.score - b.score : b.score - a.score);

        // 2. Decide Leaderboard Display Order
		let displayPlayers = isManualMode ? [...players].sort((a, b) => a.position - b.position) : [...rankedRef];
		
		// 3. DECISION: Rounds Table always follows the "mode" (High/Low sort)
        let roundPlayers = [...rankedRef];
       ...
       // document.getElementById('batchAction').style.display = players.length ? 'block' : 'none';
		...
		document.getElementById('activeGameLabel').innerHTML = activeId 
			? (hasUnsavedChanges 
			? `📁: ${activeGame?.name || "NewGame"} <span style="color: red;">${saveMe}</span>` 
		...
		displayPlayers.forEach((p) => {
            const rank = rankedRef.findIndex(rp => rp.id === p.id) + 1;
            const tr = document.createElement('tr');
            tr.setAttribute('data-id', p.id);
            tr.innerHTML = `
				<td class="drag-handle" style="${(!isManualMode || !SeatingModeOn) ? 'display:none' : ''}">☰</td>
				<td style="width:3px;"><div style="width:3px;height:52px;background:${p.color}"></div></td>
                <td style="width:30px; text-align:center; font-weight:bold;">${rank}</td>
				<td style="width:35px;"><div class="circle-color-picker" style="background: ${p.color}; 
					width: 22px; 
					height: 22px; 
					border-radius: 50%;
					/* 👇 Adds a thin white ring ONLY if the color is black 👇 */
					box-shadow: ${p.color.toLowerCase() === 'black' || p.color === '#000000' ? '0 0 0 2px #fff, 0 0 0 3px #000' : 'none'};
					" 
					onclick="openColorPicker('edit', ${p.id})"
					></div>
					</td>               


			   <td><b onclick="openRename(${p.id})">${p.name}</b></td>
                <td onclick="askDelP(${p.id})" style="width:43px; text-align:right; padding: 0px 1px 0px 0px; opacity:0.6;">🗑️</td>
				...
				<td class="col-batch" style="position: relative; width: 100px; height: 60px; text-align: right;">
  <style>
    .batch-input {
      position: absolute;
      right: 0;
      ...
      /* Base sizing and styles */
      min-width: 100px;   /* Never shrinks smaller than your original size */
      max-width: 165px;  /* Never grows too large for the table cell */
      width: 95px;
      padding: 10px;
      font-size: 2rem;
      box-sizing: border-box;
    ...
    /* Hide button when input is clicked OR when it contains numbers */
 //   .batch-input:focus + .calc-btn,
  //  .batch-input:not(:placeholder-shown) + .calc-btn {
   //   display: none !important;
   // }
  </style>

  <!-- 1. The Dynamic Input Field -->
  <input 
    type="text" 
    pattern="-[0-9]*|[0-9]*" 
    inputmode="numeric" 
    ...
    placeholder="+" 
    class="batch-input"
	style="border: 2px solid ${p.color.toLowerCase() === 'black' || p.color === '#000000' ? '#ffffff' : p.color};"

    oninput="
      const measurer = document.getElementById('measure-${p.id}');
      measurer.textContent = this.value || this.placeholder;
      
      /* Calculate width: text width + padding + border */
      const newWidth = measurer.offsetWidth + 24; 
      
      /* Clamp between min (100px) and max (165px) */
      this.style.width = Math.min(Math.max(newWidth, 100), 165) + 'px';
    "
  >
  ...
  <button 
    class="calc-btn"
    onclick="openCalc('b-${p.id}')" 
    style="position:absolute; right:.4em; top:50%; transform:translateY(-50%); font-size:0.8rem; padding: .3rem; border-radius:4px; border:none; background:#34495e; color:white; z-index: 1;"
  >CALC</button>

  <!-- 3. Hidden Text Measurer (Must match input font size exactly) -->
  <span 
    id="measure-${p.id}" 
    style="position: absolute; visibility: hidden; white-space: pre; font-size: 2rem; font-family: inherit;"
  ></span>
</td>
       	...
       	// Render Rounds Table (Uses roundPlayers - decoupled)
		// document.getElementById('roundHeader').innerHTML = `<th>R</th>` + roundPlayers.map(p => `<th style="background:${p.color}; color:#000; font-size:11px;">${p.name[0]}</th>`).join('') + `<th>✕</th>`;
        //document.getElementById('roundBody').innerHTML = rounds.map((r, ri) => `<tr><td>${ri+1}</td>${roundPlayers.map(p => `<td>${r.scores[p.id]||0}</td>`).join('')}<td onclick="askDeleteRound(${ri})">✕</td></tr>`).join('');
		// document.getElementById('roundFooter').innerHTML = players.length ? `<tr class="total-row"><td>Σ</td>${roundPlayers.map(p => `<td style="color:${p.color}">${p.score}</td>`).join('')}<td></td></tr>` : "";
        
		//document.getElementById('roundHeader').innerHTML = `<th>R</th>` + roundPlayers.map       (p => `<th style="background:${p.color}; color:black;">${p.name[0]}</th>`).join('') + `<th>✕</th>`;

		document.getElementById('roundHeader').innerHTML = `<th>R</th>` + roundPlayers.map(p => {
    // Determine text color based on background color
    const isBlackBg = p.color.toLowerCase() === 'black' || p.color === '#000000';
    const textColor = isBlackBg ? 'white' : 'black';

    return `<th style="background:${p.color}; color:${textColor};">${p.name[0]}</th>`;
  }).join('') + `<th>✕</th>`;




		//  document.getElementById('roundBody').innerHTML = rounds.map((r, ri) => `<tr><td>${ri+1}</td>${displayPlayers.map(p => `<td>${r.scores[p.id]||0}</td>`).join('')}<td onclick="askDeleteRound(${ri})">✕</td></tr>`).join('');
		//  document.getElementById('roundBody').innerHTML = rounds.map((r, ri) => `<tr><td>${ri+1}</td>${displayPlayers.map(p => `<td><span class="score-cell" onclick="openEdit(${ri}, ${p.id})">${r.scores[p.id]||0}</span></td>`).join('')}<td><button class="del-round-btn" onclick="askDelR(${ri})">✕</button></td></tr>`).join('');
        document.getElementById('roundBody').innerHTML = rounds.map((r, ri) => `<tr><td>${ri+1}</td>${roundPlayers.map(p => `<td><span class="score-cell" onclick="openEdit(${ri}, ${p.id})">${r.scores[p.id]||0}</span></td>`).join('')}<td><button class="del-round-btn" onclick="askDelR(${ri})">✕</button></td></tr>`).join('');

		// document.getElementById('roundFooter').innerHTML = players.length ? `<tr class="total-row"><td>Σ</td>${displayPlayers.map(p => `<td style="color:${p.color}">${p.score}</td>`).join('')}<td></td></tr>` : "";
		document.getElementById('roundFooter').innerHTML = players.length ? `<tr style="background:var(--total-bg); font-weight:bold;"><td>Σ</td>${roundPlayers.map(p => `<td style="color:${p.color}">${p.score}</td>`).join('')}<td></td></tr>` : "";
        ...
        if (players.length) { 
            spot.style.display = 'block'; spot.style.background = rankedRef[0].color; 
            spot.innerText = `🏆 Leader: ${rankedRef[0].name} (${rankedRef[0].score})`; 
        } else spot.style.display = 'none';
    }

    function openSaveModal() {
        let activeGame = library.find(g => g.id == activeId);
        document.getElementById('saveNameIn').value = activeGame ? activeGame.name : "NewGame";
        openModal('saveGameModal');
        setTimeout(()=>document.getElementById('saveNameIn').select(), 100);
    }

    function doSave(isNew) {
        let n = document.getElementById('saveNameIn').value || "NewGame";
        ...
        document.getElementById('libList').innerHTML = library.map((g, i) => `
            <div style="padding:12px; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center;">
                ...
                <div style="display:flex; gap:10px;">
                    <button onclick="shareG(${i})" style="background:#27ae60; color:white; border:none; border-radius:6px; padding:8px 10px;">📤</button>
                    <button onclick="delSave(${i})" style="background:#e74c3c; color:white; border:none; border-radius:6px; padding:8px 10px;">🗑️</button>
                </div>
            </div>`).join('') || `<div style="padding: 20px 12px; color: var(--text-muted);">No saved games.</div>`;
	...
	function updateVisibility2() {
		//const table = document.getElementById('leaderboard');
		const isChecked2 = document.getElementById('toggleSeating').checked;
		console.log("Value of myVariable:", isChecked2);
		if(isChecked2){
			SeatingModeOn = false;}
			else {
				SeatingModeOn = true;}
